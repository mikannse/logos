...
class Neo4jRepository:
    ...
    async def get_graph(
        ...
        V3a 改进：改为**确定性聚合**（按节点/边 distinct 汇总），替代旧的
        `RETURN path LIMIT 200` 路径式截断——后者在 depth≥2 时只返回任意
        200 条路径碰到的子集，结果不确定。构建规模有界（每跳 ≤50 节点、
        最多 3 跳），无截断需求。
        ...
        try:
            driver = await self._driver
            async with driver.session() as session:
                # 深度感知新鲜度（V3a）：图谱须在 >=depth 的深度下构建过。
                # graphDepth 缺失视为旧数据 → 未命中，触发重建（否则 depth=1
                # 构建后 depth=3 请求会静默返回 1 跳数据）。
                freshness = """
                  AND ($max_age_ms IS NULL OR center.graphBuiltAt > timestamp() - $max_age_ms)
                  AND (center.graphDepth IS NULL OR center.graphDepth >= $depth)
                """
                # 节点：遍历 1..depth 跳路径，distinct 去重（确定性聚合）
                node_query = f"""
                MATCH (center:Entity {{entityId: $entity_id}})
                WHERE center.graphBuiltAt IS NOT NULL
                  {freshness}
                MATCH path = (center)-[r*1..{depth}]-(n)
                WITH DISTINCT n
                RETURN n
                """
                # 边：遍历路径，UNWIND 所有关系，distinct 去重
                edge_query = f"""
                MATCH (center:Entity {{entityId: $entity_id}})
                WHERE center.graphBuiltAt IS NOT NULL
                  {freshness}
                ...
                node_result = await session.run(
                    node_query, entity_id=entity_id, max_age_ms=max_age_ms, depth=depth
                )
                node_records = await node_result.fetch(500)
                ...
                for rec in node_records:
                    ...
                    if eid not in node_map:
                        ...
                        node_map[eid] = {
                            ...
                            "year": props.get("year"),
                            "year_end": props.get("yearEnd"),
                            "hop": props.get("hop"),
                        }

                edge_result = await session.run(
                    edge_query, entity_id=entity_id, max_age_ms=max_age_ms, depth=depth
                )
                edge_records = await edge_result.fetch(500)

                seen_edges: set[tuple] = set()
                edges: list[dict] = []
                for rec in edge_records:
                    rel = rec["rel"]
                    a = dict(rel.start_node.items())
                    b = dict(rel.end_node.items())
                    src = a.get("entityId", "")
                    dst = b.get("entityId", "")
    ...
    async def upsert_entity(self, entity_data: dict) -> bool:
        ...
        params = {
            ...
            "year_end": entity_data.get("year_end"),
            "hop": entity_data.get("hop"),
        }
        try:
            driver = await self._driver
            async with driver.session() as session:
                await session.run(
                    ...
                    SET e.entityName = $name,
                        ...
                        e.yearEnd = coalesce($yearEnd, e.yearEnd),
                        e.hop = coalesce($hop, e.hop),
    ...
    async def mark_graph_built(self, entity_id: str, depth: int = 1) -> bool:
        """标记实体图谱构建完成时间与深度（get_graph 新鲜度判定基准）

        在 build_graph_from_wikidata 全部 upsert 成功后调用；
        未标记（构建中途失败）时 get_graph 视为未命中，下次自动重建。

        V3a: 记录构建深度 graphDepth——get_graph 的 WHERE 按
        `graphDepth IS NULL OR graphDepth >= depth` 判定，保证 depth=1
        构建的图谱不会静默服务 depth=2/3 的请求。
        """
        try:
            driver = await self._driver
            async with driver.session() as session:
    ...
    async def delete_subgraph_edges(self, entity_ids: list[str]) -> bool:
        """删除子图内部全部边（V3a 多跳重建前清理）

        多跳构建写入了 hop1→hop2、hop2→hop3 等非中心出边，
        delete_outgoing_relations 只删中心出边，清理不彻底。
        本方法删除 visited 集合内部的所有边（端点均在集合内），
        保留指向集合外（其他实体图谱）的边。

        Args:
            entity_ids: 本次构建访问到的全部节点 ID（含中心）
        """
        if not entity_ids:
            return True
        try:
            ...
            async with driver.session() as session:
                await session.run(
                    """
                    MATCH (n:Entity)
                    WHERE n.entityId IN $ids
                    MATCH (n)-[r]->(m:Entity)
                    WHERE m.entityId IN $ids
                    DELETE r
                    """,
                    ids=entity_ids,
                )
