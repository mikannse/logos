...
- [ ] 搜索"苹果公司"图谱：核心为 iPhone / 史蒂夫·乔布斯 / 蒂姆·库克 等强相关节点，不再被"公司/企业"分类节点淹没（P1-P5）
- [ ] LLM 提取实体按 `relevance>=0.5` 过滤，`llm_*` 与 Wikidata QID 合并去重（P2/P3）
- [ ] 时间轴：搜索"苹果公司"返回 ≥5 个带描述与来源的里程碑（P6）
- [ ] 缓存策略生效：首查构建后，重复查询 <500ms 命中缓存（P5/P6）
- [ ] 后端 `pytest` 通过（新增 relevance 过滤、里程碑提取、时间轴合并单测）
- [ ] 浏览器端全流程验证（搜索 → 图谱 → 时间轴 → 快照保存数据完整性）
- [ ] PRD / Epics / Architecture 文档修订已应用
